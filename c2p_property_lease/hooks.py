...
def post_init_hook(env):
    """Populate the sample portfolio on demo databases.

    A real client database has demo off, so nothing happens there - use
    Configuration -> Load Sample Portfolio instead. Both paths run the same
    generator so they cannot drift.
    """
    module = env["ir.module.module"].search([("name", "=", "c2p_property_lease")], limit=1)
    if not module.demo:
        return
    env["c2p.sample.portfolio"].load()
